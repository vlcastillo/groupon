...
def estadistica_demanda():
        ...
        for columna in datos:
            ...
            if columna[0] == "CL" and columna[1] != '':
                if columna[1] in cash_in.keys():
                    cash_in[columna[1]][dt.date(
                        *[int(columna[2]), int(columna[3]),
                          int(columna[4])])] = float(columna[6])
                else:
                    cash_in[columna[1]] = {dt.date(
                        *[int(columna[2]), int(columna[3]),
                          int(columna[4])]): float(columna[6])}
    ...
    with open("fechas_deals.csv", "r") as fechas:
        for columna in fechas:
            ...
            if columna[2] != 'NULL' and columna[0] != '' and columna[0] \
                    in categorias.keys():
                actual = dt.date(*[int(x.replace('"', '')) for x in
                                   columna[1].split(' ')[0].split('-')])
                final = dt.date(
                    *[int(x.replace('"', '')) for x in columna[2].split('-')])
...
class Categoria:
    ...
    def grafico(self):
        ...
        for i in range(len(keys)):
            ...
            cota[2 * (i + 1)] = self.separacion[key][
                0] - 0.05 * self.separacion[key][
                1] / self.separacion[key][2]
